...
export const TICKET_PRICES = { STANDARD: 850, MIDDLE_ROW: 750, BACK_ROW: 500 };

// Row capacities: P(28 usable, P29 blocked) + Q(24) + R(21) = 73
export const BACK_ROW_CAPACITY = 73; // P(28) + Q(24) + R(21)
export const MIDDLE_ROW_CAPACITY = 70; // N(33) + O(37)
export const STANDARD_CAPACITY = 502; // A–M rows

export const BACK_ROW_LETTERS = ['P', 'Q', 'R'];
export const MIDDLE_ROW_LETTERS = ['N', 'O'];
...
export const SEATING_ZONES = {
  ...
  MIDDLE_ROW: {
    name: 'Middle Row Seating',
    code: 'MID',
    color: '#7C3AED', // 🟣 Violet for middle rows N & O (₹750)
    status: 'AVAILABLE',
    description: 'Rows N & O — Middle Row Seats (₹750)',
  },
  BACK_ROW: {
    name: 'Back Row Seating',
    code: 'BACK',
    color: '#0D9488', // 🟦 Teal for back-row (₹500)
    status: 'AVAILABLE',
    description: 'Rows P, Q & R — Back Row Seats (₹500)',
  },
};
...
export function getSeatZoneAndStatus(rowLetter, num) {
  ...
  // Rows A and B — VIP (always allocatable regardless of booking tier)
  if (rowLetter === 'A' || rowLetter === 'B') {
    return { zone: SEATING_ZONES.VIP.name, status: 'LOCKED' };
  }

  // Rows N and O — Middle Row seats (₹750 tier)
  if (MIDDLE_ROW_LETTERS.includes(rowLetter)) {
    return { zone: SEATING_ZONES.MIDDLE_ROW.name, status: 'AVAILABLE' };
  }

  // Rows P, Q and R — Back Row seats (₹500 tier)
  if (BACK_ROW_LETTERS.includes(rowLetter)) {
    return { zone: SEATING_ZONES.BACK_ROW.name, status: 'AVAILABLE' };
  }
