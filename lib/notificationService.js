...
export async function sendBroadcastNotification({ channel, booking, message, attachments = [] }) {
  ...
  const eventName = process.env.EVENT_NAME || 'Nritya Bharathanjali 2026';
  ...
  if (channel === 'EMAIL') {
    ...
    // Separate image attachments (rendered inline) from file attachments (downloaded)
    const imageAttachments = attachments.filter((a) => (a.type || '').startsWith('image/'));
    const fileAttachments = attachments.filter((a) => !(a.type || '').startsWith('image/'));

    const inlineImages = imageAttachments.map((a, i) => ({
      ...a,
      cid: `broadcast-img-${i}@msnatyakshetra`,
    }));

    const inlineImagesHtml = inlineImages.length
      ? `<div style="margin: 20px 0; text-align: center;">
          ${inlineImages.map((img) => `<img src="cid:${img.cid}" alt="${img.name || 'image'}" style="max-width: 100%; border-radius: 6px; margin-bottom: 10px;" />`).join('\n')}
        </div>`
      : '';
